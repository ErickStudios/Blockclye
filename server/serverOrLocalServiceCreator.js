...
fileToLoad = fs.readFileSync(fileToLoad, "utf-8");

import { mapServerModel, mapServerModelsService, mapServerScriptService, serverOrLocalService } from "./core/serverOrLocalService.js";
import { exportLibrary, parse, run, tokenize, createEnum } from "./core/mapIndividualServices/mapServerScriptServiceLang.js";
import { argv, exit } from "node:process"
import { WebSocketServer } from "ws";
import * as os from "node:os";

function log01(msg) { console.log("  \x1b[32m•\x1b[0m", msg); }
...
let updateWorkspace = () => {
    let updateNode = /** @param {{[key: string]: any}} node */(node) => {
        if ('type' in node && typeof node.type === 'string' && node.type == 'area' && 'area' in node) {
            if (!world.areabled_parts.includes(node.area)) world.areabled_parts.push(node.area);
        }
        if ('type' in node && typeof node.type === 'string' && node.type == 'ligth' && 'ligth' in node) {
            if (!world.ligtheable_parts.includes(node.ligth)) world.ligtheable_parts.push(node.ligth);
        }
    }
    let recursive = (nodep) => {
        nodep.forEach((node) => {
        ...
        for (const v of node) {
            if (v.name == element) {
                ...
                if (v.type == 'area') {
                    ...
                    objectToSerialize.object.colides = {
                        type: 'func', func: (params) => {
                            function isInside(X, Y) {
                                ...
                                return (
                                    Xmin.x <= Ymax.x && Xmax.x >= Ymin.x &&
                                    Xmin.y <= Ymax.y && Xmax.y >= Ymin.y &&
                                    Xmin.z <= Ymax.z && Xmax.z >= Ymin.z
                                );
                            }
...
let createRuntimeLib = () => {
    ...
    lib.ButtonStyles = createEnum('ButtonStyles', ['Clasic', 'Custom']);
    lib.SimpleRectangle = {
        type: 'func', func: /** @param { any[] } params */ (params) => {
    ...
    lib.SimpleBlk = SimpleBlk;
    /** 
        interface IRigidPart extends SimpleBlk {
            var material: string;
            var weldedTo: number;
            var basePosition: Vector3;
            var baseSize: Vector3;
            var baseRotation: Vector3;
            var color: Color3;
            var mass: Vector3;
            var velocity: Vector3;
        }
        classfunc RigidPart(position?, size?, rotation?, color?, mass?, weldedTo?) extends IRigidPart {
        }
    */
    lib.RigidPart = {
        type: 'func', func: (params) => {
            let blk = SimpleBlk.func([
                lib.Vector3.func([0, 0, 0]),
                lib.Vector3.func([1, 1, 1]),
                lib.Vector3.func([0, 0, 0]),
                lib.Color3.func([1, 1, 1]),
            ]);
            blk.object.SimpleBlkModel.isStatic = false;
            params.forEach((v, i) => {
                switch (i) {
                    case 0: blk.object.SimpleBlkModel.basePosition = v.items; break;
                    case 1: blk.object.SimpleBlkModel.baseSize = v.items; break;
                    case 2: blk.object.SimpleBlkModel.baseRotation = v.items; break;
                    case 3: blk.object.SimpleBlkModel.color = v.items; break;
                    case 4:
                        if (typeof v !== "number") {
                            throw new TypeError(lib.errorAtCls('RigidPart', `param 'mass' is not a number`))
                        }
                        blk.object.SimpleBlkModel.mass = v;
                        break;
                    case 5:
                        if (!(typeof v === "number" || v === null)) {
                            throw new TypeError(lib.errorAtCls('RigidPart', `param 'weldedTo' is not a number or null`))
                        }
                        blk.object.SimpleBlkModel.weldedTo = v;
                        break;
                    default:
                        throw new TypeError(lib.errorAtCls('RigidPart', `too many params`))
                        break;
                }
            });
            return blk;
    ...
    /** 
        interface IStaticPart extends SimpleBlk {
            var material: string;
            var weldedTo: number;
            var basePosition: Vector3;
            var baseSize: Vector3;
        ...
        classfunc StaticPart(position?, size?, rotation?, color?) extends IStaticPart {
        }
    */
    lib.StaticPart = {
        type: 'func', func: (params) => {
            ...
            blk.object.SimpleBlkModel.isStatic = true;
            params.forEach((v, i) => {
                switch (i) {
                    case 0: blk.object.SimpleBlkModel.basePosition = v.items; break;
                    case 1: blk.object.SimpleBlkModel.baseSize = v.items; break;
                    ...
                    case 3: blk.object.SimpleBlkModel.color = v.items; break;
                    default:
                        throw new TypeError(lib.errorAtCls('StaticPart', `too many params`))
                        break;
                }
            });
            return blk;
...
let logo = "\x1b[1m\x1b[38;5;140mBlo\x1b[38;5;115mck\x1b[38;5;116mclye\x1b[0m";

console.log("\x1b[35m" + "=".repeat(80) + "\x1b[0m")
console.log("The " + logo + " Server Creator\x1b[0m");
console.log();
console.log("We are not responsible for the behavior on the servers, as those who host them have to intervene.")
console.log("Use your own rules to ensure the security of your Blockclye server");
console.log("" + "-".repeat(80) + "\x1b[0m")
console.log("Thank you for using Blockclye");
console.log();

console.log("\x1b[35m" + "=".repeat(80) + "\x1b[0m")

console.log("\x1b[36mServer Initing Status\x1b[0m")
log01('Server running on ws://' + getLocalIP() + ":8080");
...
function updateDeletation(deletedId, elementsToDelete = 1, serverToUpdate = world.serverOrLocalService) {
    ...
    serverToUpdate.mapServerModelsService.forEach((v, index) => {
        if (v.weldedTo === deletedId) {
            v.weldedTo = null;
        } else if (v.weldedTo > deletedId) {
            v.weldedTo -= elementsToDelete;
        }
        v.weldedTo -= (v.weldedTo > deletedId ? elementsToDelete : 0);
    })
}

world.serverOrLocalService.importJson(fileToLoad);
...
console.log("\x1b[36mServer Logs\x1b[0m")
log01('the server is ready')

wss.on("connection", (ws) => {
    log01('Player connected', ws.url);

    let id = world.serverOrLocalService.mapServerModelsService.length;

    let p = new mapServerModel();
    p.basePosition = [0, 100, 0];
    p.baseSize = [1, 2, 1];
...
function checkCollisionModel(nextPos, model, models) {
    ...
    for (let [other, index] of models.map((v, i) => [v, i])) {
        ...
        if (world.ligtheable_parts.includes(index)) continue;
...
function physicsStep(models, dt) {
    ...
    let getParent = (m) => {
        if (m.weldedTo != null && m.weldedTo != -1) return models[m.weldedTo];
        else return null;
    }

    function getRoot(m, models) {
        while (m.weldedTo != null && m.weldedTo !== -1) {
            m = models[m.weldedTo];
        }
        return m;
    }
    ...
    for (let i = 0; i < models.length; i++) {
        ...
        let cond = parent && isOnGround(parent, models);

        if (m.weldedTo != null && m.weldedTo != -1);
        ...
        let oldPos = [...m.basePosition];
        let oldVelocity = [...m.velocity];
        let velocityOldNormalized = oldVelocity.map(v => v * dt);
        let nextPos = [
        ...
        if (!collided) {
            ...
            if (cond) {
                models.move(i, ...pos, true);
            }
            else models.move(i, ...pos);
        } else {
            let push = [
                m.velocity[0],
                0,
